...
class AlquilerService {
  ...
  aceptarAlquiler(id) {
    const token = localStorage.getItem("token");
    return axios.put(`${CLIENTE_BASE_REST_API_URL}/${id}/aceptar`, null, {
      headers: { Authorization: token }
    });
  }
  ...
  rechazarAlquiler(id) {
    const token = localStorage.getItem("token");
    return axios.put(`${CLIENTE_BASE_REST_API_URL}/${id}/rechazar`, null, {
      headers: { Authorization: token }
    });
  }
  ...
  cancelarAlquiler(id) {
    const token = localStorage.getItem("token");
    return axios.put(`${CLIENTE_BASE_REST_API_URL}/${id}/cancelar`, null, {
      headers: { Authorization: token }
    });
  }
}
