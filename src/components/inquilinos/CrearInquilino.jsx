...
const CrearInquilino = () => {
  ...
  const saveOrUpdateInquilino = async (e) => {
    e.preventDefault();
    setErrores({});
    ...
    const inquilino = { nombre, apellido, email, contrasenia };

    try {
      if (id) {
        await inquilinoService.actualizarInquilino(id,inquilino)
      } else {
        await inquilinoService.createInquilino(inquilino);
      }
      navigate('/listInquilino');
    } catch (error) {
      console.error(error);
    }
  };
  ...
  useEffect(() => {
    if (id) {
        inquilinoService.getInquilinoById(id)
        .then((response) => {
  ...
  return (
    <div className="max-w-lg mx-auto bg-white p-8 rounded-2xl shadow-md">
      ...
      <form className="space-y-5" onSubmit={saveOrUpdateInquilino}>
        {/* Nombre */}
        <div>
